...
class MergeLayer(nn.Module):
    ...
    def __init__(self, input_dim1: int, input_dim2: int, hidden_dim: int,
                 output_dim: int, auxiliary_dim: int = 0):
        """
        ...
        :param auxiliary_dim: int, optional pair-level feature dimension to
            concatenate before the nonlinear hidden layer
        """
        super().__init__()
        self.auxiliary_dim = int(auxiliary_dim)
        if self.auxiliary_dim < 0:
            raise ValueError("auxiliary_dim must be non-negative")
        self.fc1 = nn.Linear(input_dim1 + input_dim2 + self.auxiliary_dim, hidden_dim)
    ...
    def forward(self, input_1: torch.Tensor, input_2: torch.Tensor,
        ...
        parts = [input_1, input_2]
        if self.auxiliary_dim > 0:
            if auxiliary_features is None:
                raise ValueError(
                    "MergeLayer was built with auxiliary_dim > 0 but no "
                    "auxiliary_features were provided"
                )
            if auxiliary_features.ndim != 2 or auxiliary_features.size(1) != self.auxiliary_dim:
                raise ValueError(
                    "auxiliary_features shape mismatch: expected "
                    f"[N, {self.auxiliary_dim}], got {tuple(auxiliary_features.shape)}"
                )
            parts.append(auxiliary_features.to(device=input_1.device, dtype=input_1.dtype))
        elif auxiliary_features is not None:
            raise ValueError(
                "auxiliary_features were provided to a MergeLayer with auxiliary_dim=0"
            )
...
class HeuristicFusionHead(nn.Module):
    """Learn an additive structural correction to a base link logit."""

    def __init__(self, feature_dim: int = 4):
        super().__init__()
        self.linear = nn.Linear(int(feature_dim), 1, bias=True)
        # Preserve the base model at initialization and let training decide how
        # much signal to take from each heuristic.
        nn.init.zeros_(self.linear.weight)
        nn.init.zeros_(self.linear.bias)

    def forward(
        self, base_logits: torch.Tensor, heuristic_features: torch.Tensor
    ) -> torch.Tensor:
        correction = self.linear(heuristic_features).squeeze(dim=-1)
