...
The default budget is five seeds (`42, 43, 44, 45, 46`) for semantic/GIN
training, Llama fine-tuning, Llama evaluation, and TabICL alignment. Router
ensembles default to `142, 143, 144, 145, 146`; temporal graph baselines retain
their five-seed default `0, 1, 2, 3, 4`.

The semantic and PEFT training entrypoints launch a separate process per seed
when neither `--seed` nor `--seeds` is supplied. `--seeds 45 46` runs only those
seeds. PEFT outputs go to `<output_dir>/seed-<seed>/`; semantic checkpoints go
to `<checkpoint_parent>/seed-<seed>/<checkpoint_name>`. Explicit `--seed 45`
keeps the single-run output path, including for checkpoint resumption.

For example, train five independent Llama adapters with:

```bash
python -m experiments.llm_lp.train_peft_link_prediction --dataset_name Enron --model_path /path/to/base-model --output_dir outputs/llama/enron --seeds 42 43 44 45 46
```

Evaluate each adapter's model separately with its matching `--seed` and
`--num_trials 1`. The evaluation entrypoint defaults to `--seed 42 --num_trials 5`,
using consecutive evaluation seeds; these trials reuse one supplied model and
...
Dataset-specific diagnostics, routing trials, encoder probes, and superseded
fusion entrypoints are available in Git history rather than the maintained tree.
