...
from utils.dygformer_heuristics import prepare_pos_neg_heuristic_tensors
...
def evaluate_model_link_prediction(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
    ...
    with torch.no_grad():
        ...
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            heuristic_auxiliary_dim = int(getattr(model[1], 'auxiliary_dim', 0))
            if heuristic_auxiliary_dim > 0:
            ...
            if heuristic_extractor is not None and heuristic_auxiliary_dim == 0:
                if len(model) < 3:
                    raise ValueError('A heuristic extractor requires a model with a heuristic fusion head.')
