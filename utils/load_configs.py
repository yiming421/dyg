...
def _parse_seed_list(seed_list: str):
    """
    Parse comma-separated seed list, e.g. "0,1,2".
    """
    if seed_list is None:
        return []
    seed_list = seed_list.strip()
    if len(seed_list) == 0:
        return []
...
def resolve_run_seeds(seed: int = None, seed_list: str = '', num_runs: int = 5):
    """
    Resolve concrete run seeds from CLI options.
    Priority: seed_list > seed > range(num_runs).
    """
    parsed_seed_list = _parse_seed_list(seed_list)
    if len(parsed_seed_list) > 0:
        return parsed_seed_list
    if seed is not None:
        return [int(seed)]
    return list(range(int(num_runs)))
...
def get_link_prediction_args(is_evaluation: bool = False):
    ...
    parser.add_argument('--model_name', type=str, default='JODIE', help='name of the model, note that EdgeBank is only applicable for evaluation',
                        choices=['JODIE', 'DyRep', 'TGAT', 'TGN', 'CAWN', 'EdgeBank', 'TCL', 'GraphMixer', 'DyGFormer']) # TGAT,CAWN (ICEWS1819) takes long time; TGN(ICEW1819) OOM
    ...
    parser.add_argument('--seed', type=int, default=None, help='single random seed for one run')
    parser.add_argument('--seed_list', type=str, default='', help='comma-separated random seeds, e.g., "0,1,2,3,4"')
    parser.add_argument('--data_seed', type=int, default=2020, help='seed controlling data split and synthetic feature initialization')
    ...
    parser.add_argument('--node_feature_path', type=str, default='',
                        help='optional .npy node-feature cache overriding the dataset e_feat.npy')
    parser.add_argument('--dygformer_attention_mode', type=str, default='learned',
                        choices=['learned', 'uniform', 'mlp_mixer'],
                        help=('DyGFormer token mixer: learned multi-head attention, fixed '
                              'padding-masked uniform attention, or a padding-safe MLP-Mixer'))
    parser.add_argument('--dygformer_padding_mode', type=str, default='masked',
                        choices=['masked', 'legacy'],
                        help='DyGFormer padding behavior: padding-safe masked mode or opt-in original legacy mode')
    parser.add_argument('--dygformer_disable_neighbor_co_occurrence',
                        action='store_false', dest='dygformer_use_neighbor_co_occurrence', default=True,
    ...
    parser.add_argument('--dygformer_heuristic_fusion_mode', type=str, default='additive',
                        choices=['additive', 'late_concat'],
                        help=('heuristic fusion: additive linear logit correction or nonlinear '
                              'concatenation before DyGFormer\'s ReLU pair head'))
    parser.add_argument('--load_best_configs', action='store_true', default=True, help='whether to load the best configurations')
    parser.add_argument('--no_load_best_configs', action='store_false', dest='load_best_configs',
                        help='keep explicitly supplied hyperparameters instead of applying model defaults')
    parser.add_argument('--dtgb_reproduction_protocol', action='store_true',
                        help='use the released DTGB fixed sampler seeds: val=0, new-val=1, test=2, new-test=3')
    parser.add_argument('--save_name_suffix', type=str, default='',
                        help='suffix appended to checkpoint/log run names')
    ...
    args.run_seeds = resolve_run_seeds(seed=args.seed, seed_list=args.seed_list, num_runs=args.num_runs)
    args.num_runs = len(args.run_seeds)
