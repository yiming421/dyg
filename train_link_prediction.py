...
from models.modules import HeuristicFusionHead, MergeLayer
...
if __name__ == "__main__":
    ...
    #args.device = 'cpu'
    run_seeds = list(args.run_seeds)

    # seed data-level randomness (splits + synthetic features)
    set_random_seed(seed=args.data_seed)
    ...
    for run_idx, run_seed in enumerate(run_seeds):
        ...
        args.seed = run_seed
        args.save_model_name = f'{args.model_name}_seed{args.seed}{args.use_feature}{args.save_name_suffix}'

        if args.dtgb_reproduction_protocol:
            # Released DTGB protocol: graph samplers and evaluation candidates are
            # fixed across model seeds. The unseeded training sampler follows the
            # per-run NumPy seed set above.
            ...
            # Deterministic run-specific seeds used by this fork's default protocol.
            train_neighbor_sampler = get_neighbor_sampler(data=train_data, sample_neighbor_strategy=args.sample_neighbor_strategy,
                                                          time_scaling_factor=args.time_scaling_factor, seed=run_seed)
            full_neighbor_sampler = get_neighbor_sampler(data=full_data, sample_neighbor_strategy=args.sample_neighbor_strategy,
                                                         time_scaling_factor=args.time_scaling_factor, seed=run_seed + 1)
            train_neg_edge_sampler = NegativeEdgeSampler(src_node_ids=train_data.src_node_ids, dst_node_ids=train_data.dst_node_ids, seed=run_seed + 2)
            val_neg_edge_sampler = NegativeEdgeSampler(src_node_ids=full_data.src_node_ids, dst_node_ids=full_data.dst_node_ids, seed=run_seed + 3)
            new_node_val_neg_edge_sampler = NegativeEdgeSampler(src_node_ids=new_node_val_data.src_node_ids, dst_node_ids=new_node_val_data.dst_node_ids, seed=run_seed + 4)
            test_neg_edge_sampler = NegativeEdgeSampler(src_node_ids=full_data.src_node_ids, dst_node_ids=full_data.dst_node_ids, seed=run_seed + 5)
            new_node_test_neg_edge_sampler = NegativeEdgeSampler(src_node_ids=new_node_test_data.src_node_ids, dst_node_ids=new_node_test_data.dst_node_ids, seed=run_seed + 6)

        train_heuristic_extractor = None
        full_heuristic_extractor = None
        if args.dygformer_use_heuristics:
            if args.model_name != 'DyGFormer':
                raise ValueError('--dygformer_use_heuristics is only supported for DyGFormer.')
            recent_cap = int(args.max_input_sequence_length) - 1
            train_heuristic_extractor = build_dygformer_heuristic_extractor(
        ...
        logger.info(f"********** Run {run_idx + 1} starts (seed={run_seed}). **********")
        ...
        if args.model_name == 'TGAT':
            ...
            dynamic_backbone = DyGFormer(node_raw_features=node_raw_features, edge_raw_features=edge_raw_features, neighbor_sampler=train_neighbor_sampler,
                                         ...
                                         attention_mode=args.dygformer_attention_mode,
                                         use_neighbor_co_occurrence=args.dygformer_use_neighbor_co_occurrence,
                                         padding_mode=args.dygformer_padding_mode,
                                         device=args.device)
        else:
            raise ValueError(f"Wrong value for model_name {args.model_name}!")
        heuristic_auxiliary_dim = (
            len(HEURISTIC_FEATURE_NAMES)
            if args.dygformer_use_heuristics
            and args.dygformer_heuristic_fusion_mode == 'late_concat'
            else 0
        )
        link_predictor = MergeLayer(input_dim1=node_raw_features.shape[1], input_dim2=node_raw_features.shape[1],
                                    hidden_dim=node_raw_features.shape[1], output_dim=1,
                                    auxiliary_dim=heuristic_auxiliary_dim)
        model_parts = [dynamic_backbone, link_predictor]
        if (args.dygformer_use_heuristics
                and args.dygformer_heuristic_fusion_mode == 'additive'):
            model_parts.append(
                HeuristicFusionHead(feature_dim=len(HEURISTIC_FEATURE_NAMES))
            )
        ...
        for epoch in range(args.num_epochs):
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                if args.model_name in ['TGAT', 'CAWN', 'TCL']:
                    ...
                    raise ValueError(f"Wrong value for model_name {args.model_name}!")
                positive_heuristics = None
                negative_heuristics = None
                if train_heuristic_extractor is not None:
                    positive_heuristics, negative_heuristics = prepare_pos_neg_heuristic_tensors(
                        extractor=train_heuristic_extractor,
                        pos_sources=batch_src_node_ids,
                        pos_targets=batch_dst_node_ids,
                        pos_times=batch_node_interact_times,
                        neg_sources=batch_neg_src_node_ids,
                        neg_targets=batch_neg_dst_node_ids,
                        neg_times=batch_node_interact_times,
                        device=batch_src_node_embeddings.device,
                        dtype=batch_src_node_embeddings.dtype,
                    )
                if args.dygformer_heuristic_fusion_mode == 'late_concat':
                    positive_logits = model[1](
                        input_1=batch_src_node_embeddings,
                        input_2=batch_dst_node_embeddings,
                        auxiliary_features=positive_heuristics,
                    ).squeeze(dim=-1)
                    negative_logits = model[1](
                        input_1=batch_neg_src_node_embeddings,
                        input_2=batch_neg_dst_node_embeddings,
                        auxiliary_features=negative_heuristics,
                    ).squeeze(dim=-1)
                else:
                ...
                if (train_heuristic_extractor is not None
                        and args.dygformer_heuristic_fusion_mode == 'additive'):
                    positive_logits = model[2](positive_logits, positive_heuristics)
                    negative_logits = model[2](negative_logits, negative_heuristics)
        ...
        logger.info(f'Run {run_idx + 1} (seed={run_seed}) cost {single_run_time:.2f} seconds.')
